@if ($image->isBeforeAfterHaveTwoImages == 0)
    <div class="col-md-4" id="service_extra_image_{{ $image->id }}">
        <div class="mb-3">
            <div class="">
                <a href="javascript:void(0);" title="{{ $image->image_title }}"
                    onclick="openServiceExtraImageZoomModal('{{ asset_uploads($folder . '/' . $image->image_name . '?' . time()) }}');">
            ...
            <div class="image_btn mt-2">
                <div class="drag service_extra_images_sortable_div" title="Drag and Drop to sort"><i class="fas fa-arrows"
                        aria-hidden="true"></i></div>
                <a title="Delete Image"
                    onclick="deleteServiceExtraImage({{ $image->id, '\'' . $image->image_name . '\'' }}, this);"
                ...
                @if ((bool) $image->isBeforeAfter == false)
                    <a onClick="markServiceExtraImageBeforeAfter({{ $image->id }}, this)" href="javascript:void(0)"
                        class="mb-1 btn btn-warning">Mark Before
                        After</a>
                @endif
                <a title="Crop Image" onClick="bind_cropper_preview_service_extra_image({{ $image->id }}, 1);"
                    href="javascript:void(0)" class="mb-1 btn btn-warning"><i class="fas fa-crop"
                        aria-hidden="true"></i></a>
                <a title="Image Alt/Title" onClick="openServiceExtraImageAltTitleModal({{ $image->id }});"
...
@else
    <div class="col-md-12" id="service_extra_image_{{ $image->id }}">
        <div class="row">
            <div class="col-md-4">
                <div class="mb-3">
                    <div class="">
            ...
            <div class="col-md-4">
                <div class="mb-3">
                    <div class="">
                        <a href="javascript:void(0);" title="{{ $image->image_title }}"
                            onclick="openServiceExtraImageZoomModal('{{ asset_uploads($folder . '/' . $image->image_name2 . '?' . time()) }}');">
            ...
            <div class="col-md-12 text-center">
                <div class="mb-3">
                    ...
                    <div class="image_btn mt-2">
                        ...
                        @endif
                        <a title="Crop Image"
                            onClick="bind_cropper_preview_service_extra_image({{ $image->id }}, 1);"
                            href="javascript:void(0)" class="mb-1 btn btn-warning"><i class="fas fa-crop"
                                aria-hidden="true"></i></a>
                        <a title="Crop Image"
                            onClick="bind_cropper_preview_service_extra_image({{ $image->id }}, 2);"
                            href="javascript:void(0)" class="mb-1 btn btn-warning"><i class="fas fa-crop"
                                aria-hidden="true"></i></a>
                        <a title="Image Alt/Title" onClick="openServiceExtraImageAltTitleModal({{ $image->id }});"
                            href="javascript:void(0)" class="mb-1 btn btn-success"><i class="fas fa-bars"
