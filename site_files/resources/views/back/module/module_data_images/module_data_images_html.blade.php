<div class="row" style="display:{{ $module->show_more_images_option == 1 ? 'block' : 'none' }}">
    <div class="col-md-12">
        <hr />
        <input type="hidden" name="session_id" value="{{ session()->getId() }}" />
        <input type="hidden" name="module_data_id" value="{{ isset($moduleData) ? $moduleData->id : 0 }}" />

        <label class="text-sm text text-info more_images_label"><i class="fas fa-image" aria-hidden="true"></i> More
            Images (<em>Max :</em> {{ getMaxUploadSize() }} MB)</label>
        ...
        <div class="m-2 row sortable_row more_images" id="moreImages" style="display:none;">
            @foreach ($moduleDataImages as $image)
                @include('back.module.module_data_images.module_data_images_html_sub', [
                    'folder' => 'module/' . $module->type,
                    'image' => $image,
                ])
            @endforeach
        </div>
