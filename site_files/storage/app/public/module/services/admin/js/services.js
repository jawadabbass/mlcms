...
function load_services_edit_form(id){
	...
	$.getJSON(base_url +'services/get_cms_by_id/'+id, function(data) {
                ...
                if (data.image == null || data.image=='' || data.image==0){
                    ...
                    $("#featured_img").fadeIn();
                    var img = '<div class="featured-images-main" id="products_img_'+data.ID+'"><img style="width:150px" src="'+asset_uploads+'services/thumb/'+data.image+'"><i onClick="remove_services_featured_image('+data.ID+');" class="deleteIcon"></i></div>';
                    $('#featured_img').html('');                
