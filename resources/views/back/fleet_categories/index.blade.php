                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="box-header">
                                    <h3 class="box-title">All Fleet Categories</h3>
                                </div>
                            </div>                            
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body table-responsive">
                            <form method="post" id="fleetCategory-search-form">
                                <div class="row mb-3">
                                    <div class="col-lg-4">
                                    ...
                                    <div class="col-sm-8 text-end">
                                        <div class="text-end" style="padding-bottom:2px;">
                                            <a href="{{ route('fleetCategories.create') }}" class="sitebtn">Add Fleet Category</a>
                                        </div>
                                        <a href="{{ route('fleetCategories.sort') }}" class="btn btn-warning">
                                            <i class="la la-bars"></i>Sort Fleet Categories
                                        </a>
                                    </div>
                                ...
                                <div class="row mb-3" id="filterForm" style="display: none;">
                                    ...
                                    <div class="col-md-3 form-group">
                                        ...
                                        <select class="form-control" name="status" id="status">
                                            {!! generateFleetCategoriesStatusDropDown(request('status', '')) !!}
                                        </select>
                                    </div>
                                </div>
                                <table class="table table-striped table-bordered" style="width: 100%"
                                    id="fleetCategoryDatatableAjax">
                                    <thead>
                                        <tr>
                                            <th>Image</th>
                                            <th>Title</th>
...
@section('beforeBodyClose')
    <script>
        $(function() {
            var oTable = $('#fleetCategoryDatatableAjax').DataTable({
                "autoWidth": true,
                processing: true,
                serverSide: true,
                stateSave: true,
                ...
                info: true,
                ajax: {
                    url: '{!! route('fetchFleetCategoriesAjax') !!}',
                    data: function(d) {
                        d.title = $('#title').val();
            ...
            $('#fleetCategory-search-form').on('submit', function(e) {
                oTable.draw();
                e.preventDefault();
            });
        ...
        function deleteFleetCategory(id) {
            var msg = 'Are you sure?';
            var url = '{{ url('adminmedia/fleetCategories/') }}/' + id;
            if (confirm(msg)) {
                $.post(url, {
                        id: id,
                        _method: 'DELETE',
        ...
        function updateFleetCategoryStatus(id, prev_status, status) {
            var url = '{{ url('adminmedia/updateFleetCategoryStatus/') }}';
            var msg = 'Are you sure?';
            if (confirm(msg)) {
                $.post(url, {
                        id: id,
